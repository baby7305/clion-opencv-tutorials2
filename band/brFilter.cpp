...
String brFilterspectrum = "band against map";
...
enum BRFILTER_TYPE {
    IBR_FILTER = 0, BBR_FILTER = 1, GBR_FILTER = 2
};
            ...
            for (int c = 0; c < cols; c++) {
                ...
                if (norm2 != pow(_radius, 2))
                    _bpFilter.at<float>(r, c) =
                            1.0 / (1.0 + pow(sqrt(norm2) * _bandWidth / (norm2 - pow(_radius, 2)), 2 * n));
                else
                    _bpFilter.at<float>(r, c) = 0;
            }
        }
    ...
    if (type == GBR_FILTER) {
        for (int r = 0; r < rows; r++) {
            for (int c = 0; c < cols; c++) {
                ...
                if (norm2 != 0)
                    _bpFilter.at<float>(r, c) =
                            1.0 - exp(-pow((norm2 - pow(_radius, 2)) / (_bandWidth * sqrt(norm2)), 2));
                else
                    _bpFilter.at<float>(r, c) = 1;
            }
        }
