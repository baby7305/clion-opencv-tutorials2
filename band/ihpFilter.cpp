...
int main(int argc, char *argv[]) {
    ...
    imshow("fourier map", fImageFFT_spectrum);
    //得到傅里叶谱最大值的位置
    minMaxLoc(fImageFFT_spectrum, NULL, NULL, NULL, &maxLoc);
    /*理想高通滤波器*/
    namedWindow("imagine high pass map", 1);
    createTrackbar("半径:", "imagine high pass map", &radius, Max_RADIUS, callback_ihpFilter);
    waitKey(0);
    return 0;
}
...
void callback_ihpFilter(int, void *) {
    ...
    imshow("imagine high pass map", fImageFFT_ihpFilter_spectrum);
    ...
    imshow("imagine high pass fourier map", result);
