...
void sepConv2D_X_Y(InputArray src, OutputArray src_kerX_kerY, int ddepth, InputArray kernelX, InputArray kernelY,
...
void sepConv2D_Y_X(InputArray src, OutputArray src_kerY_kerX, int ddepth, InputArray kernelY, InputArray kernelX,
                   Point anchor = Point(-1, -1), int borderType = BORDER_DEFAULT) {
    //输入矩阵与垂直方向卷积核的卷积
    Mat src_kerY;
...
void callback_thresh(int, void *) {
    ...
    imshow("threshold edge strengh", thresh_edge);
}
...
int main(int argc, char *argv[]) {
    ...
    imshow("perpendicular edge", abs_img_prewitt_x);
    //imwrite("img1_v_edge.jpg", abs_img_prewitt_x);
    imshow("horizontal edge", abs_img_prewitt_y);
    ...
    imshow("edge strengh", edge);
    //imwrite("img1_edge.jpg", edge);
    Mat tempedge;
    threshold(edge, tempedge, 25, MAX_THRSH, cv::THRESH_BINARY);
    imwrite("img3_thresh_edge_25.jpg", tempedge);
    //阈值处理后的边缘强度
    namedWindow("threshold edge strengh", cv::WINDOW_AUTOSIZE);
    createTrackbar("threshold", "threshold edge strengh", &Thresh, MAX_THRSH, callback_thresh);
    callback_thresh(0, 0);
    waitKey(0);
    return 0;
}