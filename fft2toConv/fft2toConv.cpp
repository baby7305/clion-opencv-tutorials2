...
Mat fft2Conv(Mat I, Mat kernel, int borderType = BORDER_DEFAULT, Scalar value = Scalar()) {
    ...
    copyMakeBorder(I_padded, I_padded_zeros, 0, rows - I_padded.rows, 0, cols - I_padded.cols, BORDER_CONSTANT,
                   Scalar(0, 0, 0, 0));
    copyMakeBorder(kernel, kernel_zeros, 0, rows - kernel.rows, 0, cols - kernel.cols, BORDER_CONSTANT,
                   Scalar(0, 0, 0, 0));
    /* 第三步：快速傅里叶变换 */
